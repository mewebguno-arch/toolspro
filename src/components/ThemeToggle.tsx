  ...
  return (
    <div className="relative group">
      <button
        id={id}
        onClick={toggleTheme}
        className="p-2.5 rounded-full bg-surface dark:bg-[#1A1A2E] border border-border dark:border-[#2D2D45] text-text-base hover:bg-muted/10 transition-all duration-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background cursor-pointer flex items-center justify-center active:scale-95"
        aria-label={theme === 'light' ? 'Switch to dark mode' : 'Switch to light mode'}
      >
        <div className="transition-transform duration-250 active:scale-90">
          {theme === 'light' ? (
